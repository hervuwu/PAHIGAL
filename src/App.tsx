...
function App() {
  ...
  useEffect(() => {
    // 1. Curated Jewel Tone Palettes
    const aestheticPalettes = [
      { name: "Royal Amethyst", grad: "-45deg, #2b0359, #540d9c, #8b1cd1, #2b0359", sent: "#8b1cd1" },
      { name: "Sapphire Abyss", grad: "-45deg, #091353, #16248c, #2a41d6, #091353", sent: "#2a41d6" },
      { name: "Emerald Canopy", grad: "-45deg, #023618, #07612f, #10944b, #023618", sent: "#10944b" },
      { name: "Ruby Flare", grad: "-45deg, #4a0016, #800026, #c21543, #4a0016", sent: "#c21543" },
      { name: "Oceanic Bioluminescence", grad: "-45deg, #04103a, #0b346e, #1377a5, #04103a", sent: "#1377a5" },
      { name: "Ember Glow", grad: "-45deg, #3d0c02, #781d04, #c43c08, #3d0c02", sent: "#c43c08" }
    ];

    // 2. Subtle CSS Patterns (using low opacity so they don't distract)
    const patterns = [
      // Blueprint Grid
    ...
    document.documentElement.style.setProperty('--primary-gradient', `linear-gradient(${randomPalette.grad})`);
    document.documentElement.style.setProperty('--sent-color', randomPalette.sent);
    document.documentElement.style.setProperty('--bg-pattern', randomPattern.css);
    document.documentElement.style.setProperty('--bg-pattern-size', randomPattern.size);
  }, []);
  ...
  const connect = () => {
    ...
    socket.onmessage = (event) => {
      ...
      if (message.type === "waiting") {
        ...
        setPartnerName(name);
        setMessages([{ text: `You've been matched with ${name}!`, sender: "them", timestamp: formatTime(), author: "System" }]);
      } else if (message.type === "partner-disconnected") {
        // Updated to use partnerName directly
        setMessages((prev) => [...prev, { text: `${partnerName} has disconnected.`, sender: "them", timestamp: formatTime(), author: "System" }]);
        disconnectChat();
      } else if (message.text) {
        setIsPartnerTyping(false);
        setMessages((prev) => [...prev, { text: message.text, sender: "them", timestamp: message.timestamp || formatTime(), author: message.username || partnerName }]);
      }
    };
  ...
  const handleMessageSubmit = (e?: any) => {
    ...
    if (input.trim() && ws.current?.readyState === WebSocket.OPEN) {
      ...
      ws.current.send(JSON.stringify({ text: input, username, timestamp: time }));
      setMessages((prev) => [...prev, { text: input, sender: "me", timestamp: time, author: username }]);
      setInput("");
    }
  };
  ...
  return (
    <div className={`app-wrapper ${darkMode ? "dark-mode" : ""}`}>
      ...
      {reconnectAttempts > 0 && reconnectAttempts < 5 && (
        <div className="toast-notification" style={{ background: "var(--sent-color)" }}>
          <span className="toast-icon">🔄</span> Reconnecting... (Attempt {reconnectAttempts})
        </div>
      )}
      ...
      {status === "UNSET" && (
        <div className="username-container">
          ...
          <form onSubmit={handleUsernameSubmit} className="username-input">
            <input type="text" value={username} onChange={(e) => setUsername(e.target.value)} placeholder="Who are you?" maxLength={15} required />
            <button type="submit" className="primary-btn" disabled={status === "WAITING"}>Find Someone</button>
          </form>
        </div>
      )}
      ...
      {status === "WAITING" && (
        <div className="username-container">
          ...
          <button className="new-match-btn" onClick={() => { 
        ...
        <div className="chat-container">
          <div className="chat-header">
            <div className="header-left">
              <span className="brand-name-small" onClick={() => { 
                statusRef.current = "UNSET"; 
                setStatus("UNSET");
                if (ws.current) { ws.current.close(); ws.current = null; } 
              }}>PAHIGAL</span>
          ...
          <div className="messages">
            {messages.map((msg, index) => (
              <div key={index} className={`message-wrapper ${msg.sender === "me" ? "sent-wrapper" : "received-wrapper"}`} onClick={() => setVisibleTimestampIndex(visibleTimestampIndex === index ? null : index)}>
                <div className={`message ${msg.sender === "me" ? "sent" : "received"}`}>{msg.text}</div>
          ...
          <form onSubmit={handleMessageSubmit} className={`message-input ${status === "DISCONNECTED" ? "disabled-input" : ""}`}>
            <input type="text" value={input} onChange={handleInputChange} disabled={status === "DISCONNECTED"} placeholder={status === "CHATTING" ? "Type a message..." : "Chat ended."} autoFocus />
